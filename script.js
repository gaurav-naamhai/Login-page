...
let prev=document.getElementById("bo1");
let next=document.getElementById("bo2");
...
const searchFunc=(e)=>{clearTimeout(searchTimer);
    searchTimer=setTimeout(()=>{
        state.serachQuery=e.target.value.toLowerCase();
    updateUI();
    state.currentPage=1;
    },300);
};
const sorting=(e)=>{state.sortOrder=e.target.value;
    updateUI();
    state.currentPage=1;
};
const saveUsers=()=>{
...
const renderUser=()=>{
userList.innerHTML="";
 const filteredUsers=state.users.filter(u=>u.email.includes(state.serachQuery));
 ...
 const end=start+state.itemperPage;
 const paginatedUsers=sortedUsers.slice(start,end);
const totalPages=Math.ceil(sortedUsers.length/state.itemperPage);
page.innerText=`${state.currentPage}/${totalPages}`;
const total=state.users.length;
const filtered=sortedUsers.length;
const showingForm=sortedUsers.length===0?0:start+1;
const showingTo=Math.min(end,filtered);
stats.innerText=`Showing ${showingForm}-${showingTo} of ${filtered}(Total:${total})`;
...
next.addEventListener("click",()=>{
    const visible = state.users.filter(u=>u.email.includes(state.serachQuery));
 const maxPage = Math.ceil(visible.length/state.itemperPage);
